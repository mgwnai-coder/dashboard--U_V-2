...
# القفل الأول: 5 في الدقيقة
@ratelimit(key='ip', rate='5/m', block=False)
# القفل الثاني: 20 في الساعة
@ratelimit(key='ip', rate='20/h', block=False)
# القفل الثالث: 50 في اليوم
@ratelimit(key='ip', rate='50/d', block=False)
# 🌟 دالة تسجيل الدخول (محمية بالأقفال الثلاثة) 🌟
# التعديل: أضفنا method='POST' لكي لا يتم حظر مجرد تحديث الصفحة (GET)
...
def admin_login(request):
    ...
    if getattr(request, 'limited', False):
        ...
        # 🌟 التعديل هنا: أزلنا status=429 لكي لا يتعارض المتصفح مع تحديث الـ CSRF
